      ...
      <tbody class=" text-right">
        ...
        @if ($Today_appointment->ActiveType==5)
            ...
            <tr class="bg-warning" ><td  class="bg-warning"></td>
              <td  class="bg-warning text-color">@php $debut = date('H:i', strtotime($Today_appointment->debut));
                echo "محجوز ";
                @endphp</td>
        ...
        @endif>
          ...
          <td  class="align-middle clearfix" style="position: relative;"><img class=" border rounded-circle ml-2" width="50" height="50" src="{{$picture}}" alt="">
            ...
            overflow: hidden;
            white-space: nowrap;
          text-overflow: ellipsis;"><a  class="text-color" href="sendMsg/{{$Today_appointment->fb_id}}">{{$Today_appointment->facebook}}</a></span>  <span dir="ltr" style=" position: absolute;
            top:1px;
            font-size:10px;
...
هل تريد حقا إلغاء هذا الموعد ؟         </div>
        <div class="modal-footer row float-right text-right">
         ...
         <form action="/annulerByAdmin" id="myForm" method="post"> 
            ...
            <a  href="/annulerByAdmin/" class="btn btn-danger  text-color col-4"> نعم </a>

            <a class="btn btn-secondary text-color col-4" data-dismiss="modal">  لا شكرا </a>
          </form> </div> 
